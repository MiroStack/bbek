...
import { hideCreateOffering } from "../../../datasource/redux/modules/church_record/OfferingSlice";
...
export const CreateOffering = ({setIsRefreshing}:CreateOfferingProps) => {
    ...
    async function handleSaveDonation(e:FormEvent) {
        e.preventDefault();
        dispatch(showLoader());
        try {
            ...
            const response = await offeringRepo.submitOffering(offeringModel, false);
            if (response.statusCode == 201) {
                sessionStorage.setItem("message", response.message);
                dispatch(showSuccessDialog());
                dispatch(hideCreateOffering());
                setMemberName("");
                setAmount(0);
                setDate("");
                setNotes("");
    ...
    return (
        <>
            ...
            >
                ...
                <button
                    type="button"
                    onClick={() => dispatch(hideCreateOffering())}
