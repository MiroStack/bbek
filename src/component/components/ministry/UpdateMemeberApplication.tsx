...
import { hideUpdateMemberApplication, hideUpdateMinistry } from "../../../datasource/redux/modules/church_record/MinistrySlice";

// NEW IMPORTS
import { FiChevronDown, FiX } from "react-icons/fi";

type UpdateMemeberApplicationFormProps = {
  pivotId: number;
  statusName: string;
  setIsRefresh: React.Dispatch<React.SetStateAction<boolean>>;
};

export const UpdateMemeberApplicationForm = ({
  pivotId,
  statusName,
  setIsRefresh,
}: UpdateMemeberApplicationFormProps) => {
  ...
  async function handleUpdateApplicationStatus(e: FormEvent) {
    ...
    try {
      const response = await ministryRepo.updateMemberApplication(
        pivotId,
        status,
      );

      if (response.statusCode === 200) {
        dispatch(hideUpdateMemberApplication());
    ...
    <>
      ...
      >
        <div className="flex flex-col text-center sm:text-left">
          <h2 className="text-lg font-semibold">Update Ministry</h2>
        ...
        <button
          type="button"
          onClick={() =>  dispatch(hideUpdateMemberApplication())}
