...
interface ActionModel {
    type: string;
    payload: PaginatedOfferingModel[];
}
...
const reducer = (state: PaginatedOfferingModel[], action: ActionModel) => {
    ...
    switch (action.type) {
        ...
        case OfferingTypes.tithe:
            //    console.log("tithe");
            return action.payload.filter((item, index) => item.offeringType == 1);
        case OfferingTypes.offering:
            //    console.log("offering");
            return action.payload.filter((item, index) => item.offeringType == 2);
        case OfferingTypes.special_offering:
            return action.payload.filter((item, index) => item.offeringType == 3);
        default:
            //    console.log("all");
            return state;
    }
...
export const TithesOfferingAdminPage = () => {
    ...
    const [allOffering, setAllOffering] = useState<PaginatedOfferingModel[]>([]);
    ...
    const [selectedItem, setSelectedItem] = useState<PaginatedOfferingModel | null>(null);
    ...
    const offeringService = OfferingService({
        allOffering,
        pageNumber,
        totalPage,
        query,
    ...
    useEffect(() => {
        if (isRefreshing) {
            offeringService.getOferringTypeResponse();
            offeringService.getPaymentMethodResponse();
    ...
    return (
        <>
            ...
            <div className={`${warningDialog}? '':'hidden' `}>
                <WarningDialog onConfirm={offeringService.handleDeleteOffering} />
            </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-4 justify-between">
                                <div className="relative w-full md:w-1/3">
                                    ...
                                    <input
                                        ...
                                        placeholder="Search..."
                                        value={query}
                                        onChange={offeringService.handleQueryChange}
                                        type="text"
                                        id="search"
                                    />
                                </div>
                                <div className="flex gap-2 relative">
                                    ...
                                    <button
                                        ...
                                        type="button"
                                        onClick={offeringService.handleShowSorting}
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="24"
                                            ...
                                            <tr key={index} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                                ...
                                                <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0">
                                                    <span className="px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800">{
                                                        offeringType.find((offering) => offering.id === item.offeringType)?.offeringType || "Unknown"
                                                    }</span>
                                                </td>
                                                <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 hidden md:table-cell">
                                                    {
                                                        paymentMethods.find((payment) => payment.id === item.paymentMethod)?.paymentMethod || "Unknown"
                                                    }
                                                </td>
                                                <td className="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 hidden md:table-cell">
                                                    {item.notes}
