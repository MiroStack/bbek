...
export const BaptismRepo = () => {
  ...
  return {
    ...
    async getBaptism(
      query: string,
      page: number
    ): Promise<ApiResponseModel<BaptismResponseModel[]>> {
      const token = Cookies.getCookie("auth_token");
      const response = await axios.get<
        ApiResponseModel<BaptismResponseModel[]>
      >(`getBaptism?query=${query}&page=${page}`, {
        headers: {
    ...
    async getOfficiants(): Promise<ApiResponseModel<UserAndIdModel[]>> {
      const token = Cookies.getCookie("auth_token");
      const response = await axios.get<ApiResponseModel<UserAndIdModel[]>>(
        `getOfficiants`,
        {
          headers: {
            "Content-Type": "application/json",
    ...
    async getCertificateStatuses(): Promise<
      ApiResponseModel<CertificateStatusModel[]>
    > {
      const token = Cookies.getCookie("auth_token");
      const response = await axios.get<
        ApiResponseModel<CertificateStatusModel[]>
    ...
    async getBaptismStatuses(): Promise<
      ApiResponseModel<BaptismStatusModel[]>
    > {
      const token = Cookies.getCookie("auth_token");
      const response = await axios.get<ApiResponseModel<BaptismStatusModel[]>>(
        `getBaptismStatuses`,
        {
          headers: {
            "Content-Type": "application/json",
